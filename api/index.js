// api/index.js - BRONX OSINT v2 ULTRA PRIME - ALL IN ONE
...
function initDefaultData() {
    ...
    [{ key:'jeetop',name:'🎁 Premium',scopes:['*'],limit:1000,cooldown:0,expiry:'31-12-2027' },{ key:'MoonWitch2',name:'🎁 Basic',scopes:['vehicle'],limit:500,cooldown:2,expiry:'30-06-2027' },{ key:'MoonWitch3',name:'🎁 Starter',scopes:['number','ip','pincode'],limit:200,cooldown:1,expiry:'31-12-2027' },{ key:'MoonWitch4',name:'🎁 OP',scopes:['*'],limit:999,cooldown:0,expiry:'31-12-2027' },{ key:'MoonWitch5',name:'🎁 Pro',scopes:['*'],limit:5000,cooldown:0,expiry:'31-12-2028' },{ key:'MoonWitch6',name:'🎁 Bomber',scopes:['number','custom'],limit:300,cooldown:3,expiry:'31-12-2027' }].forEach(d => { keyStorage[d.key] = { name:d.name,scopes:d.scopes,type:'demo',limit:d.limit,used:0,cooldown:d.cooldown||0,expiry:parseExpiryDate(d.expiry),expiryStr:d.expiry,created:now,unlimited:false,hidden:false }; });
}
...
const apiExamples = {
    number:{req:'GET /api/key-moonwitch/number?key=YOUR_KEY&num=7307841587',res:'{"success":true,"number":"7307841587","results":[{"name":"Nemsingh","mobile":"7307841587","circle":"JIO UPE"}]}'},
    aadhar:{req:'GET /api/key-moonwitch/aadhar?key=YOUR_KEY&num=393933081942',res:'{"success":true,"aadhar":"393933081942","results":[{"name":"J Vinod","phoneNumber":"9490160194"}]}'},
    name:{req:'GET /api/key-moonwitch/name?key=YOUR_KEY&name=abhiraaj',res:'{"success":true,"name":"abhiraaj","results":[{"name":"ABHIRAAJ GAWADE","phoneNumber":"9823796702"}]}'},
    upi:{req:'GET /api/key-moonwitch/upi?key=YOUR_KEY&upi=example@ybl',res:'{"success":true,"upi_id":"example@ybl","account_name":"MURENDRA SARABU","bank":"Union Bank"}'},
    ifsc:{req:'GET /api/key-moonwitch/ifsc?key=YOUR_KEY&ifsc=SBIN0001234',res:'{"success":true,"ifsc":"SBIN0001234","bank":"State Bank of India","branch":"HAJIGANJ"}'},
    pincode:{req:'GET /api/key-moonwitch/pincode?key=YOUR_KEY&pin=110001',res:'{"success":true,"pincode":"110001","state":"Delhi","district":"Central Delhi"}'},
    ip:{req:'GET /api/key-moonwitch/ip?key=YOUR_KEY&ip=8.8.8.8',res:'{"success":true,"ip":"8.8.8.8","country":"United States","city":"Mountain View"}'},
    leakinfo:{req:'GET /api/key-moonwitch/leakinfo?key=YOUR_KEY&info=email@example.com',res:'{"status":"success","data":{"rc_number":"MH02FZ0555","owner_name":"SHAH RUKH KHAN"}}'},
    vehicle:{req:'GET /api/key-moonwitch/vehicle?key=YOUR_KEY&vehicle=MH02FZ0555',res:'{"status":"success","data":{"rc_number":"MH02FZ0555","owner_name":"SHAH RUKH KHAN"}}'},
    ff:{req:'GET /api/key-moonwitch/ff?key=YOUR_KEY&uid=123456789',res:'{"success":true,"uid":"123456789","info":{"Nickname":"MoonWitch","Level":"67"}}'},
    insta:{req:'GET /api/key-moonwitch/insta?key=YOUR_KEY&username=cristiano',res:'{"success":true,"username":"cristiano","profile":{"followers":672571267}}'},
    tg:{req:'GET /api/key-moonwitch/tg?key=YOUR_KEY&info=JAUUOWNER',res:'{"success":true,"info":"JAUUOWNER","number":"9627507420","country":"India"}'},
};
...
app.get('/test',(req,res)=>{res.json({status:'✅ MoonWitch OSINT v2',keys:Object.keys(keyStorage).filter(k=>!keyStorage[k]?.hidden).length})});
...
function renderLogin(){return `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>MoonWitch v2 | ADMIN</title><link href="https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Rajdhani:wght@500;700&display=swap" rel="stylesheet"><style>
*{margin:0;padding:0;box-sizing:border-box}body{background:#0a0a0f;display:flex;justify-content:center;align-items:center;min-height:100vh;font-family: Arial, Helvetica, sans-serif;overflow:hidden}
...
.box::before{content:'MoonWitch OSINT v2';display:block;text-align:center;font-family:'Orbitron',sans-serif;font-size:11px;letter-spacing:6px;background:linear-gradient(90deg,#8b00ff,#00c8ff,#ff0080);-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:10px;font-weight:900}
...
.box input{width:100%;padding:16px 18px;background:rgba(0,0,0,.4);border:1px solid rgba(255,255,255,.08);border-radius:12px;color:#fff;margin-bottom:14px;font-size: 16px;outline:none;font-family: Arial, Helvetica, sans-serif;transition:.4s}
...
</style></head><body><div class="particles" id="pt"></div><div class="box"><h2>ADMIN ACCESS</h2><p class="sub">v2 ULTRA PRIME</p><input type="text" id="u" placeholder="USERNAME"><input type="password" id="p" placeholder="PASSWORD"><button class="btn" onclick="login()">AUTHENTICATE</button><p class="msg" id="msg"></p><p class="fp">MoonWitch OSINT v2 · GOD TIER</p></div><script>
...
return `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>MoonWitch v2 | ADMIN PANEL</title><link href="https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Rajdhani:wght@400;500;600;700&display=swap" rel="stylesheet"><style>
:root{--bg:#06060c;--sur:#0d0d1a;--brd:rgba(139,0,255,.15);--txt:#e0e0f0;--acc:#8b00ff;--acc2:#00c8ff;--green:#00ff88;--red:#ff0080;--yellow:#ffaa00}
*{margin:0;padding:0;box-sizing:border-box}body{background:var(--bg);color:var(--txt);font-family: Arial, Helvetica, sans-serif;font-size:13px;min-height:100vh}
...
.tb{display:flex;gap:8px;flex-wrap:wrap;align-items:center}.tb button{background:rgba(139,0,255,.08);color:var(--txt);border:1px solid var(--brd);padding:8px 14px;border-radius:8px;cursor:pointer;font-size:11px;font-family: Arial, Helvetica, sans-serif;font-weight:600;letter-spacing:1px;transition:.4s}
...
.fg input,.fg select{width:100%;padding:11px 14px;background:var(--bg);border:1px solid var(--brd);border-radius:10px;color:#fff;font-size:12px;font-family: Arial, Helvetica, sans-serif;outline:none;transition:.4s}
...
</style></head><body><div class="top"><h1>MoonWitch</h1><div class="tb"><span style="color:#444;font-size:10px">[ ${getIndiaDateTime()} ]</span><button onclick="window.open('/')">🏠 HOME</button><button onclick="window.open('/docs')">📚 DOCS</button><button onclick="location.href='/admin'">🚪 LOGOUT</button></div></div><div class="ct">
...
<div class="pn" id="pn-ips"><div class="sn"><h3>🛡 IP MANAGER</h3><div style="margin-bottom:10px"><input id="bip" placeholder="Enter IP..." style="padding:10px;background:var(--bg);border:1px solid var(--brd);color:#fff;font-family: Arial, Helvetica, sans-serif;width:220px;font-size:11px;border-radius:8px"><button class="ab a-r" onclick="banIP2()" style="margin-left:8px;padding:10px 16px;font-size:11px">🚫 BAN</button></div><div style="max-height:350px;overflow:auto"><table><tr><th>IP</th><th>REQS</th><th>STATUS</th><th>ACT</th></tr>${ipHTML}</table></div></div></div>
...
function renderDocs(){try{let html='';const cats={};Object.entries(endpoints).forEach(([n,e])=>{if(!cats[e.c])cats[e.c]=[];cats[e.c].push({name:n,...e});});html+='<div class="hero"><h1>📚 API DOCS v2</h1></div><div class="st"><div class="sc"><div class="v">'+Object.keys(endpoints).length+'</div><div class="l">ENDPOINTS</div></div></div>';Object.entries(cats).forEach(([c,eps])=>{html+=`<div class="cat-sec"><h2>${c.toUpperCase()}</h2><div class="ep-grid">`;eps.forEach(e=>{const ex=apiExamples[e.name]||{req:'GET /api/key-moonwitch/'+e.name+'?key=KEY&'+e.p+'='+e.e,res:'{"success":true}'};html+=`<div class="ep-card"><span class="ep-method">GET</span><b>/${e.name}</b><p>${e.d}</p><code class="req">${esc(ex.req)}</code><pre class="res">${esc(ex.res)}</pre></div>`});html+='</div></div>'});return `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>MoonWitch API DOCS v2</title><link href="https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Rajdhani:wght@400;500;600;700&display=swap" rel="stylesheet"><style>
:root{--bg:#06060c;--sur:#0d0d1a;--brd:rgba(139,0,255,.15);--txt:#e0e0f0;--acc:#8b00ff;--acc2:#00c8ff;--green:#00ff88}
*{margin:0;padding:0;box-sizing:border-box}body{background:var(--bg);color:var(--txt);font-family: Arial, Helvetica, sans-serif;font-size: 16px;min-height:100vh}
body::before{content:'';position:fixed;inset:0;background:radial-gradient(ellipse at 50% 0%,rgba(139,0,255,.06),transparent 60%),radial-gradient(ellipse at 80% 100%,rgba(0,200,255,.04),transparent 50%);pointer-events:none;z-index:0}
...
.ep-card b{color:#fff;font-size: 16px;margin-left:6px}.ep-card p{color:#666;font-size:10px;margin:4px 0}
...
*{margin:0;padding:0;box-sizing:border-box}body{background:var(--bg);color:var(--txt);font-family: Arial, Helvetica, sans-serif;overflow-x:hidden;font-size:14px}
...
.tb .logo{font-family:'Orbitron',sans-serif;font-size: 16px;letter-spacing:4px;background:linear-gradient(90deg,var(--acc),var(--acc2),var(--pink));-webkit-background-clip:text;-webkit-text-fill-color:transparent;font-weight:900}
...
.pc:hover{border-color:var(--acc);background:rgba(139,0,255,.06)}.pc .d{font-size:10px;color:#fff;font-weight:600}.pc .a{font-size: 16px;color:var(--acc);font-weight:900}
...
.upi .pay-btn-glow{display:inline-block;background:linear-gradient(135deg,var(--acc),var(--acc2),var(--pink));background-size:300% 100%;animation:rb 3s linear infinite;color:#fff;padding:12px 30px;border-radius:25px;font-size: 16px;font-weight:800;text-decoration:none;cursor:pointer;box-shadow:0 0 30px rgba(139,0,255,.3);transition:.3s;letter-spacing:1px;border:none;font-family:'Orbitron',sans-serif}
...
.pl h3{font-size: 16px;font-weight:700;margin-bottom:10px;background:linear-gradient(135deg,var(--acc2),#fff);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
...
.ep b{font-size: 16px;color:#fff;display:block;margin-bottom:2px}.ep small{font-size:9px;color:#666;display:block;margin-bottom:6px}
...
</style></head><body><div class="snow" id="snow"></div><div class="tb"><a href="/" class="logo">⚡ MoonWitch v2</a><div style="display:flex;gap:10px;align-items:center"><a href="/docs">📚 DOCS</a><a href="/admin">🔐 ADMIN</a><span class="ba">✅ PAID API</span></div></div><header class="hero"><h1>MoonWitch OSINT v2</h1><p class="sub">ULTRA PRIME SUITE</p><p style="color:#555;font-size:10px;margin-top:4px">All Lookups | 24/7 Support | 0 Error</p></header><div class="ct"><div class="st"><div><div class="v">${totalEndpoints}</div><div class="l">ENDPOINTS</div></div><div><div class="v">${totalKeys}</div><div class="l">DATABASES</div></div><div><div class="v">UNLIMITED</div><div class="l">REQUESTS</div></div><div><div class="v">100%</div><div class="l">WORKING</div></div></div><div class="pay"><h3>💎 GET PAID API KEY</h3><div class="pg"><div class="pc"><div class="d">1 Day</div><div class="a">₹20</div></div><div class="pc"><div class="d">5 Days</div><div class="a">₹50</div></div><div class="pc"><div class="d">10 Days</div><div class="a">₹100</div></div><div class="pc"><div class="d">30 Days</div><div class="a">₹300</div></div><div class="pc"><div class="d">50 Days</div><div class="a">₹500</div></div><div class="pc"><div class="d">60 Days</div><div class="a">₹600</div></div><div class="pc" style="grid-column:1/-1;border-color:rgba(204,93,232,.2)"><div class="d">PERMANENT KEY</div><div class="a" style="color:#cc5de8">₹3000</div></div></div><div class="pm"><span class="pb">📱 UPI</span><span class="pb">💳 PhonePe</span><span class="pb">🏦 Navi</span><span class="pb">💰 Paytm</span></div><div class="upi"><a href="upi://pay?pa=8509561376@ibl&pn=MoonWitch&cu=INR" class="pay-btn-glow">⚡ CLICK TO PAY NOW ⚡</a><img src="qr.png" alt="QR" onerror="this.style.display='none'"></div><p class="tgl">After Payment , Message Here → <a href="https://t.me/moonwitchservices">@moonwitchadminbot</a></p></div><div class="pl"><h3>🧪 API PLAYGROUND</h3><div class="plf"><select id="es"><option value="">Select Endpoint</option>${opts}</select><input id="ak" placeholder="API Key..."><input id="pv" placeholder="Parameter..."><button class="btx" onclick="ta()">⚡ RUN</button></div><div class="rb" id="rb"></div></div><div class="eg">${cardsHTML}</div></div><footer class="ft"><p class="fb">MoonWitch OSINT v2 👑</p><p class="fi">@moonwitchadminbot · <a href="/admin">Admin</a> · <a href="/docs">API Docs</a> · <a href="/test">Status</a></p></footer><script>
var eps=${epsJSON};
...
(async function(){const loaded=await loadFromStorage();if(!loaded){initDefaultData();initCustomAPIs()}if(!keyStorage[MASTER_API_KEY])keyStorage[MASTER_API_KEY]=createMasterKey();delete keyStorage['MOONWITCH_MASTER_2026'];scheduleSave();console.log('✅ MoonWitch OSINT v2 ULTRA PRIME Ready! Keys:',Object.keys(keyStorage).length)})();
