...
function renderHome(){try{const vapis=customAPIs.filter(a=>a.visible&&a.endpoint);const totalEndpoints=Object.keys(endpoints).length+vapis.length;const totalKeys=Object.keys(keyStorage).filter(k=>!keyStorage[k]?.hidden).length;const epsJSON=JSON.stringify(endpoints).replace(/</g,'\\u003c');let cardsHTML='';Object.entries(endpoints).forEach(([n,e])=>{cardsHTML+=`<div class="ep" onclick="cp('${esc(n)}','${esc(e.p)}','${esc(e.e)}')"><span>${e.i}</span><b>/${esc(n)}</b><small>${esc(e.d)}</small><code>${esc(e.p)}=${esc(e.e)}</code></div>`});vapis.forEach(a=>{cardsHTML+=`<div class="ep" onclick="ccp('${esc(a.endpoint)}','${esc(a.param)}','${esc(a.example)}')"><span>🔧</span><b>/${esc(a.endpoint)}</b><small>${esc(a.desc||'Custom')}</small><code>${esc(a.param)}=${esc(a.example||'v')}</code></div>`});const opts=Object.entries(endpoints).map(([n,e])=>`<option value="${esc(n)}" data-p="${esc(e.p)}" data-ex="${esc(e.e)}">${e.i} /${esc(n)}</option>`).join('')+vapis.map(a=>`<option value="c_${a.id}" data-c="1" data-ep="${esc(a.endpoint)}" data-p="${esc(a.param)}" data-ex="${esc(a.example)}">🔧 /${esc(a.endpoint)}</option>`).join('');return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>MoonWitch OSINT V100</title><style>
:root{--bg:#0a0a0f;--sur:#16161f;--txt:#e0e0e0;--acc:#8b00ff;--acc2:#00c8ff}
*{margin:0;padding:0;box-sizing:border-box}
body{background:var(--bg);color:var(--txt);font-family:Arial,sans-serif;font-size:16px;line-height:1.6}
.tb{padding:20px;display:flex;justify-content:space-between;border-bottom:1px solid #333}
.tb .logo{font-size:20px;font-weight:bold;color:#fff}
.tb a{color:#aaa;text-decoration:none;font-size:16px;margin-left:20px}
...
.st>div{text-align:center;padding:20px;background:var(--sur);border:1px solid #333;border-radius:8px;flex:1}
.st .v{font-size:24px;font-weight:bold;color:var(--acc2)}
.st .l{font-size:14px;color:#888}
.pl{background:var(--sur);border:1px solid #333;border-radius:8px;padding:20px;margin-bottom:30px}
.pl h3{font-size:20px;margin-bottom:15px;color:#fff}
.plf{display:flex;gap:10px;flex-wrap:wrap}
.plf select,.plf input{flex:1;min-width:200px;padding:12px;border:1px solid #333;border-radius:5px;font-size:16px;background:#0a0a0f;color:#fff}
.btx{padding:12px 25px;background:var(--acc);color:#fff;border:none;border-radius:5px;cursor:pointer;font-size:16px;font-weight:bold}
.rb{margin-top:15px;background:#000;padding:15px;border-radius:5px;color:#00ff88;font-family:monospace;display:none;white-space:pre-wrap;font-size:14px}
.eg{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:15px}
.ep{background:var(--sur);border:1px solid #333;border-radius:8px;padding:20px;cursor:pointer;transition:0.2s}
.ep:hover{border-color:var(--acc2)}
.ep b{display:block;font-size:18px;margin-bottom:5px;color:#fff}
.ep small{font-size:14px;color:#aaa;display:block;margin-bottom:8px}
.ep code{font-family:monospace;font-size:13px;background:rgba(0,0,0,0.3);padding:3px 6px;border-radius:4px;color:var(--acc2)}
.ft{text-align:center;padding:40px;color:#777;font-size:14px}
</style></head><body>
<div class="tb"><div class="logo">MoonWitch OSINT V100</div><div><a href="/docs">DOCS</a><a href="/admin">ADMIN</a></div></div>
<header class="hero"><h1>MoonWitch OSINT V100</h1></header>
<div class="ct">
<div class="st">
