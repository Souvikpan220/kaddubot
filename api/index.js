...
:root{--bg:#f4f4f9;--sur:#ffffff;--brd:#ddd;--txt:#333;--acc:#4a90e2;--acc2:#4a90e2;--green:#27ae60;--red:#e74c3c;--yellow:#f39c12}
*{margin:0;padding:0;box-sizing:border-box}body{background:var(--bg);color:var(--txt);font-family:Arial, Helvetica, sans-serif;font-size:16px;line-height:1.5}
.tb{position:sticky;top:0;z-index:1000;background:#fff;border-bottom:1px solid var(--brd);padding:15px 20px;display:flex;justify-content:space-between;align-items:center}
.tb .logo{font-size:18px;font-weight:bold;color:var(--acc);text-decoration:none}
.tb a{color:#555;text-decoration:none;font-size:14px;margin-left:15px}.tb a:hover{color:var(--acc)}
.hero{text-align:center;padding:50px 20px}
.hero h1{font-size:40px;margin-bottom:10px}
.hero .sub{font-size:16px;color:#777;margin-bottom:20px}
.ct{max-width:1100px;margin:0 auto;padding:0 20px}
.st{display:flex;justify-content:center;gap:30px;padding:20px;background:#fff;border:1px solid var(--brd);border-radius:10px;margin-bottom:20px}
.st>div{text-align:center}.st .v{font-size:24px;font-weight:bold;color:var(--acc)}.st .l{font-size:12px;color:#888;text-transform:uppercase}
.pay{background:#fff;border:1px solid var(--brd);border-radius:10px;padding:20px;margin-bottom:20px}
.pay h3{font-size:20px;text-align:center;margin-bottom:15px}
.pg{display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:10px;margin-bottom:15px}
.pc{background:#f9f9f9;border:1px solid var(--brd);border-radius:6px;padding:10px;text-align:center}.pc .d{font-size:12px}.pc .a{font-size:16px;font-weight:bold}
.upi{text-align:center;padding:15px}.upi .pay-btn-glow{display:inline-block;background:var(--acc);color:#fff;padding:12px 25px;border-radius:5px;text-decoration:none;font-weight:bold}
.pl{background:#fff;border:1px solid var(--brd);border-radius:10px;padding:20px;margin-bottom:20px}
.pl h3{font-size:16px;margin-bottom:10px}
.plf{display:flex;gap:10px;flex-wrap:wrap}.plf select,.plf input{flex:1;padding:10px;border:1px solid var(--brd);border-radius:5px;font-size:14px}
.btx{padding:10px 20px;background:var(--acc);color:#fff;border:none;border-radius:5px;font-weight:bold;cursor:pointer}
.rb{margin-top:10px;background:#f9f9f9;padding:10px;border:1px solid var(--brd);font-size:12px;display:none;white-space:pre-wrap}
.eg{display:grid;grid-template-columns:repeat(auto-fill,minmax(250px,1fr));gap:15px;margin-bottom:20px}
.ep{background:#fff;border:1px solid var(--brd);border-radius:8px;padding:15px;cursor:pointer;border-top:4px solid var(--ac,#8b00ff)}
.ep b{font-size:16px;display:block}.ep small{font-size:12px;color:#666;display:block;margin-bottom:5px}
.ep code{background:#eee;padding:2px 4px;border-radius:3px;font-size:12px}
.ft{text-align:center;padding:20px;color:#888;font-size:12px}
</style></head><body><div class="tb"><a href="/" class="logo">MoonWitch V100</a><div><a href="/docs">Docs</a><a href="/admin">Admin</a></div></div><header class="hero"><h1>MoonWitch OSINT V100</h1><p class="sub">ULTRA PRIME SUITE</p></header><div class="ct"><div class="st"><div><div class="v">${totalEndpoints}</div><div class="l">Endpoints</div></div><div><div class="v">${totalKeys}</div><div class="l">Keys</div></div></div><div class="pay"><h3>Get Premium Access</h3><div class="pg"><div class="pc"><div class="d">10 Days</div><div class="a">₹100</div></div><div class="pc"><div class="d">30 Days</div><div class="a">₹300</div></div><div class="pc" style="grid-column:1/-1"><div class="d">LIFETIME</div><div class="a">₹3000</div></div></div><div class="upi"><a href="upi://pay?pa=8509561376@ibl&pn=MoonWitch&cu=INR" class="pay-btn-glow">Click to Pay</a></div></div><div class="pl"><h3>API Playground</h3><div class="plf"><select id="es"><option value="">Select Endpoint</option>${opts}</select><input id="ak" placeholder="API Key..."><input id="pv" placeholder="Parameter..."><button class="btx" onclick="ta()">Run</button></div><div class="rb" id="rb"></div></div><div class="eg">${cardsHTML}</div></div><footer class="ft"><p>© 2026 MoonWitch OSINT V100</p></footer><script>
var eps=${epsJSON};
async function ta(){var s=document.getElementById('es'),o=s.options[s.selectedIndex],k=document.getElementById('ak').value,v=document.getElementById('pv').value,rb=document.getElementById('rb');if(!s.value||!k||!v){alert('Fill all fields');return}var url=o.dataset.c==='1'?'/api/custom/'+o.dataset.ep+'?key='+k+'&'+o.dataset.p+'='+v:'/api/key-kaddu/'+s.value+'?key='+k+'&'+eps[s.value].p+'='+v;rb.style.display='block';rb.textContent='Loading...';try{var r=await fetch(url);var d=await r.json();rb.textContent=JSON.stringify(d,null,2)}catch(e){rb.textContent='Error: '+e.message}}
